...
The flow below is for **feature skills** (branch-based). For utility skills (self-contained tools), the contributor opens a PR that adds files directly to `.claude/skills/<name>/` — no branch extraction needed. See [CONTRIBUTING.md](../CONTRIBUTING.md) for all skill types.
