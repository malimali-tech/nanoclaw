...
async function doRefreshToken(
  ...
  if ((code !== undefined && code !== 0) || error) {
    ...
    if (REFRESH_TOKEN_RETRYABLE.has(code as number)) {
      log.warn(
        `refresh transient error (code=${errCode}) for ${opts.userOpenId}, retrying once`,
      );
      ...
      log.warn(
        `refresh failed (code=${errCode}), clearing token for ${opts.userOpenId}`,
      );
      await removeStoredToken(opts.appId, opts.userOpenId);
      return null;
    }
  ...
  log.info(
    `refreshed UAT for ${opts.userOpenId} (at:${maskToken(updated.accessToken)})`,
  );
  return updated;
}
