...
//                                         conversation.
//
// We deliberately do NOT scan the host user's `~/.agents/skills/` —
// that directory is shared across every Claude project on the machine,
// and pulling it into NanoClaw means a feishu agent ends up with the
// user's personal `frontend-design` / `web-design-guidelines` skills in
// its system prompt. NanoClaw's catalog ships in-repo under
// `groups/global/skills/`; want to add a skill, commit it there.
